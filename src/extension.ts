  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('skorezy.showPanel', () => {
      DetailPanel.show(manager?.matches ?? [], {
        ...
        footballMatchDetail: footballProvider
          ? (lc: string, id: string) => footballProvider!.getMatchDetail(lc, id)
          : undefined,
...
function bootstrap(context: vscode.ExtensionContext): void {
  ...
  const allProviders: Record<string, () => ScoreProvider> = {
    ...
    football: () => new FootballProvider(cfg.footballLeagues, cfg.footballFavoriteTeams),
